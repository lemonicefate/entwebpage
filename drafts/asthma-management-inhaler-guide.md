...
status: draft
seo_description: "氣喘發作怎麼辦？了解緊急處置步驟、吸入劑常見錯誤，以及居家環境控制技巧，結合 GINA 2025 最新指引，幫助你有效控制氣喘、減少急性發作。"
...
```mermaid
flowchart TD
    A[反覆咳嗽/喘鳴/胸悶] --> B{症狀持續 > 4 週？}
    B -->|否| C[觀察，記錄發作時機與誘發因子]
    B -->|是| D[就診進行肺功能評估]
    D --> E{肺量計 + 支氣管擴張劑測試}
    E -->|FEV₁ 改善 ≥ 12%| F[確診氣喘，開始階梯治療]
    E -->|結果不明確| G[支氣管激發試驗 or 試驗性治療]
    F --> H[定期追蹤、調整用藥階梯]
```
...
```mermaid
flowchart TD
    A[確診氣喘] --> B[Step 1: 低劑量 ICS]
    B --> C{控制良好？}
    C -->|是| D[維持，每3-6月追蹤是否降階]
    C -->|否| E[Step 2: 低劑量 ICS/LABA 複方]
    E --> F{控制良好？}
    F -->|是| G[維持]
    F -->|否| H[Step 3: 中高劑量 ICS/LABA]
    H --> I{仍控制不良？}
    I -->|是| J[評估加入生物製劑或會診胸腔科]
    I -->|否| K[維持，定期追蹤]
```
